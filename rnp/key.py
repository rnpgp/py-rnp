...
class Key:
    ...
    def bits_25519_tweaked(self):
        """Advanced: check whether the secret key's bits are correctly clamped
        for Curve25519 (X25519) usage, i.e. the 3 least significant bits are
        zero and the key is exactly 255 bits, as per RFC 7748 section 5.

        Returns True if the bits are correctly set, False if they still need
        tweaking (see bits_25519_tweak()). Note: rnp.h documents this value
        inverted; the returned value follows the actual librnp behaviour.

        Requires an unlocked ECDH Curve25519 secret key (i.e. the encryption
        subkey of an Ed25519/X25519 keypair); raises RnpException otherwise.
        """
        result = c_bool()
        _lib.rnp_key_25519_bits_tweaked(self._obj, byref(result))
        return result.value

    def bits_25519_tweak(self):
        """Advanced: clamp the Curve25519 (X25519) secret key's least/most
        significant bits in place, as per RFC 7748 section 5. The operation is
        idempotent. Keys generated by RNP are clamped at generation time, so
        this is only needed for imported keys with unclamped secret bits,
        making the exported secret key compatible with implementations which
        do not tweak the bits automatically.

        Requires an unprotected ECDH Curve25519 secret key (the secret key
        data is modified, so call protect() afterwards if needed); raises
        RnpException otherwise.
        """
        _lib.rnp_key_25519_bits_tweak(self._obj)
